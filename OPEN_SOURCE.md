...
单独发布 `codex-mobile-web` 仓库，源码已经是独立 Git 仓库，上一级目录中的旧试验项目不并入。公开仓库为 [hzx829/codex-mobile-web](https://github.com/hzx829/codex-mobile-web)，项目采用 [MIT 许可证](LICENSE)。
...
1. **项目名和许可。** 已确定使用 Codex Mobile Web 与 MIT；源码和两个发布包均包含项目许可证，package 元数据记录许可与仓库。包内另附依赖许可证，Windows 包携带 Node 许可证。
2. **把真实部署过程变成可复用说明。** 补齐目前已跑通的 Node + systemd 路径，去掉个人 IP、目录和凭据；公开推荐入口统一为 HTTPS。现有 Docker + Caddy 路径若作为推荐入口，应先由人类完成一次实际部署确认。
3. **收口文档和发布包。** QUICKSTART、ACCEPTANCE 等文件的旧状态已同步，未逐项确认的用例继续保留。两个包携带双语 README、隐私说明和许可证，从确定提交生成，不直接发布正在使用的安装目录。应用实测由人类完成，记录简短结果即可。
