...
服务器只运行网页、中继和 HTTPS 入口；Codex、模型凭据、项目文件与会话历史留在开发电脑。中继没有业务数据库。Caddy 的数据卷只保存证书和相关配置。
...
服务器需保留 `.env` 和 Caddy 证书卷，电脑需保留自己的 `.local/`。程序包中不包含这些文件。中继不记录聊天或文件正文，代理未启用访问日志；容器运行日志做大小轮换。
...
当前交付完成部署包与本地程序检查。尚无服务器和域名，公网证书签发、Linux 容器运行及手机跨网操作留到真实部署时验证。
