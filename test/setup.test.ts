...
test('configuration preserves identity and Codex settings, matches local port, and requires remote credentials',async()=>{
  ...
  try {
    ...
    const first=await prepareSettings({relayUrl:'http://127.0.0.1:4444'});
    ...
    assert.equal(next.roots,undefined);assert.equal(next.profile,old.profile);assert.equal(next.codexHome,project);
    ...
    assert.equal((await prepareSettings({roots:['Z:/no-longer-exists']},old)).roots,undefined);
...
test('damaged settings are not silently replaced and connection page escapes local values',async()=>{
  ...
  try {
    ...
    assert.ok(!page.includes('&lt;project&gt;'));assert.ok(page.includes('全部项目和会话'));
