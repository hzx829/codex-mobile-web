...
<section>已接入项目<ul>${config.roots.map((root:string)=>`<li>${escape(root)}</li>`).join('')}</ul><p>更改地址、Token 或项目：先完成任务并运行 stop.cmd，再打开 configure.cmd。电脑休眠或断网时无法远程操作。</p><small>二维码包含连接凭证，请保留在自己的设备中。模型 Key 仍由电脑上的 Codex 管理。</small></section>
<script>
