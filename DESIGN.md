...
命令、文件、权限、用户问题和基本 MCP 表单有独立回应路径；嵌套表单和 URL 外部授权交回电脑。手机不设置一套额外权限。Windows 安装采用带 Node 的便携 ZIP，首次用本机窗口配置项目、地址、Token 和已有 Codex 路径；本地状态页显示连接、模型与二维码。可选当前用户登录自启，沿用所选连接方式。修改配置前结束任务并停止本安装；升级采用新目录加保留 .local 的手工方式，旧目录用于回退。自动升级、托盘和 macOS 打包暂未实现。
