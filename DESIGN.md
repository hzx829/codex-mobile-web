...
状态：0.1 实现已落地，文中范围仍包含待验收及后续补齐项。更新日期：2026-09-17。产品范围见 [中文 README](README.zh-CN.md)，实际数据与权限边界见 [隐私说明](PRIVACY.md#简体中文)，开发顺序见 [实施计划](IMPLEMENTATION.md)。
...
首页为 Remote：顶部显示电脑连接状态，正文先列项目，再列最近会话；会话标题保持单行，右侧显示相对时间。底部悬浮搜索和新聊天入口。项目来源为 Codex 原生桌面项目列表、原生配置与会话目录，沿用桌面名称和顺序，不另设项目白名单，搜索交由电脑的原生会话索引处理，保留分页。

项目会话列表默认只读 Codex 原生状态索引，避免每次打开和轮询都扫描历史文件。手动“刷新项目与会话”才触发原生历史核对，再从索引返回统一的排序和分页游标。同一项目、搜索和归档条件只允许一个列表请求在途；轮询等待该请求结束，切换条件后丢弃旧结果。
...
局域网使用同一 Wi-Fi，不依赖 Tailscale；默认地址排除 VPN 与虚拟网卡。公网提供独立预构建包，Linux Docker Compose 运行中继，Caddy 提供 HTTPS/WSS；已有反向代理时只启动中继。程序随包分发，服务器只拉取基础镜像，不编译 npm 依赖。固定 Compose 项目名维持升级时的容器与证书卷身份，具体操作见 [公网部署](DEPLOY.md)。

一个中继实例服务一个用户自己的电脑和手机。用户自行部署并信任该中继，首版不建设共享托管平台。连接成功后默认接入对应电脑 Codex 的全部项目和会话，不区分只读、操作员或管理员。

访问控制只保留一个随机生成的实例 Token。电脑连接器与手机使用相同 Token；WebSocket 认证通过后才能请求电脑/会话数据及操作，网页、静态资源和 `/health` 公开可访问。不增加账号、邀请制、设备空间、逐设备批准或权限表。二维码用于传递连接配置，无需手机安装独立 App。
...
文件引用绑定机器与会话，相对路径按该会话实际工作目录解析，绝对路径直接读取。沿用电脑当前用户的文件读取权限，不额外限制项目根或拒绝跨目录引用；模型 Key 不作为配置展示字段转发。文本大小和主动内容限制用于预览及传输。
...
命令、文件、权限、用户问题和基本 MCP 表单有独立回应路径；嵌套表单和 URL 外部授权交回电脑。手机不设置一套额外权限。Windows 安装采用带 Node 的便携 ZIP，首次用本机窗口配置地址、Token 和已有 Codex 路径，项目与会话自动发现；本地状态页显示连接、模型与二维码。可选当前用户登录自启，沿用所选连接方式。修改配置前结束任务并停止本安装；升级采用新目录加保留 .local 的手工方式，旧目录用于回退。自动升级、托盘和 macOS 打包暂未实现。
